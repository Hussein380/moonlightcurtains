...
export default function Home() {
  ...
  const products: any[] = Array.isArray(rawProducts) 
    ? [...rawProducts].sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()).slice(0, 4) 
    : [];
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {[
              ...
              <Link href={`/shop?roomType=${cat.name.replace(' Curtains', '')}`} key={i} className="group relative h-80 md:h-96 overflow-hidden rounded-xl shadow-md">
